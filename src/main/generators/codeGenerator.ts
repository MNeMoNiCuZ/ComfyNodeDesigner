 ...
 */
function generateNodeClass(node: ComfyNodeDef): string {
  const lines: string[] = []
  ...
  // RETURN_TYPES
  const returnTypes = node.outputs.map((o) => (o.type === 'COMBO' ? 'STRING' : o.type))
  ...
  if (node.executeBody && node.executeBody.trim() !== '' && node.executeBody.trim() !== 'pass') {
    // The executeBody comes in with 8-space indent expected; ensure it
    const body = node.executeBody
    lines.push(body)
  } else {
    // Return tuple or empty
    if (node.outputs.length > 0) {
      const returnVars = node.outputs.map((o) => `result_${o.name.toLowerCase().replace(/\s+/g, '_')}`)
      const none_assigns = returnVars.map((v) => `        ${v} = None  # TODO: implement`).join('\n')
      lines.push(none_assigns)
 ...
 */
function generateNodesPy(nodes: ComfyNodeDef[]): string {
  const parts: string[] = []
  ...
  for (const node of nodes) {
    parts.push(generateNodeClass(node))
    parts.push(``)
  }
 ...
 */
function generateInitPy(nodes: ComfyNodeDef[], packageMode: boolean): string {
  const lines: string[] = []
  ...
  if (packageMode) {
    lines.push(`from .nodes import (`)
    nodes.forEach((n) => lines.push(`    ${n.internalName},`))
    lines.push(`)`)
  } else {
    nodes.forEach((n) => lines.push(`from .nodes import ${n.internalName}`))
  }
 ...
 */
function generateSingleFilePy(nodes: ComfyNodeDef[]): string {
  const parts: string[] = []
 ...
 */
export function generateAllFiles(nodes: ComfyNodeDef[], projectName = 'my_comfy_nodes'): GeneratedFiles {
  if (nodes.length === 0) {
    return {
  ...
  return {
    nodesPy: generateNodesPy(nodes),
    initPy: generateInitPy(nodes, true),
    requirementsTxt: generateRequirementsTxt(),
    readmeMd: generateReadmeMd(nodes, projectName),
    singleFilePy: generateSingleFilePy(nodes)
  }
}
