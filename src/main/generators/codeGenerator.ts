...
export interface GeneratedFiles {
  ...
  singleFilePy: string
  /** maps node.internalName → individual .py file content */
  nodeFiles: Record<string, string>
  /** __init__.py that imports from individual node files */
  initPyIndividual: string
}
...
/**
 * Generate a standalone .py file for a single node.
 */
export function generateIndividualNodeFile(node: ComfyNodeDef, packName?: string): string {
  const parts: string[] = []
  ...
  parts.push(generateNodeClass(node, packName))
  parts.push(``)
  parts.push(`NODE_CLASS_MAPPINGS = {"${node.internalName}": ${node.internalName}}`)
  parts.push(`NODE_DISPLAY_NAME_MAPPINGS = {"${node.internalName}": "${node.displayName}"}`)
  parts.push(`__all__ = ["NODE_CLASS_MAPPINGS", "NODE_DISPLAY_NAME_MAPPINGS"]`)
  parts.push(``)
  return parts.join('\n').trimEnd() + '\n'
}
...
/**
 * Generate __init__.py that imports from individual node files (one per node).
 */
function generateInitPyIndividual(nodes: ComfyNodeDef[]): string {
  const lines: string[] = []
  ...
  if (nodes.length === 0) {
    ...
    const mSpread = nodes.map((_, i) => `**_m${i}`).join(', ')
    const dSpread = nodes.map((_, i) => `**_d${i}`).join(', ')
    lines.push(`NODE_CLASS_MAPPINGS = {${mSpread}}`)
    lines.push(`NODE_DISPLAY_NAME_MAPPINGS = {${dSpread}}`)
  }
...
export function generateAllFiles(
  ...
  if (nodes.length === 0) {
    return {
      ...
      nodeFiles: {},
      initPyIndividual: generateInitPyIndividual([])
    }
  }

  const nodeFiles: Record<string, string> = {}
  for (const node of nodes) {
    nodeFiles[node.internalName] = generateIndividualNodeFile(node, packName)
  }
  ...
  return {
    ...
    nodeFiles,
    initPyIndividual: generateInitPyIndividual(nodes)
  }
}
