...
export function SettingsModal({ open, onClose }: SettingsModalProps): JSX.Element {
  const { llm, setProviderModel, setProviderBaseUrl } = useSettingsStore()
  ...
  async function handleFetchOllamaModels(): Promise<void> {
    ...
    try {
      const models = await window.electronAPI.fetchOllamaModels(
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <Tabs defaultValue="openai">
          ...
          {PROVIDERS.map((provider) => {
            const config = llm.providers[provider]
            const models = [...DEFAULT_MODELS[provider], ...(provider === 'ollama' ? ollamaModels : [])]
